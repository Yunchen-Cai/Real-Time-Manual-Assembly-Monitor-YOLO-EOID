...
coco_instance_id_to_name = {
    1: "Overall display board",
    2: "Right display board",
    3: "car",
    4: "motorcycle",
    5: "Screw",
    6: "bus",
...
hoi_interaction_names = [
    ...
    'hunt', 'inspect', 'install', 'jump', 'kick', 'kicks', 'Install the overall display board', 'Press the right display board', 'Pick up the overall display board',
    'lick', 'Pick up screws', 'Push out the screws', 'light', 'load', 'Install the display panel screws', 'make', 'milk', 'move',
    'Install the heating plate', 'open', 'operate', 'pack', 'paint', 'park', 'pay',
    'peel', 'Install the heating plate screws', 'pick', 'pick_up', 'point', 'Install the power connector', 'pull', 'push', 'race',
    'Cover the back cover', 'Press the back cover', 'repair', 'ride', 'row', 'run', 'sail', 'scratch',
    'Pick up the base plate screws', 'Install the base plate screws', 'shear', 'sign', 'sip', 'sit_at', 'sit_on', 'slide',
    'Install the rear center screw', 'spin', 'squeeze', 'stab', 'stand_on', 'stand_under', 'stick',
    'stir', 'Mark the cover', 'Install the cover screws', 'tag', 'talk_on', 'Install the inner pot',
    'text_on', 'throw', 'tie', 'Install the steam valve', 'train', 'turn', 'type_on', 'Install the upper inner lid',
    'wash', 'watch', 'wave', 'Install the handle', 'wield', 'zip',
...
def get_args_parser():
    ...
    parser.add_argument('--img_sheet', default=None, type=str, help='Path to the image sheet or a single image')
    ...
    # HOI
    parser.add_argument('--num_obj_classes', type=int, default=53,
                        help="Number of object classes")
    parser.add_argument('--num_verb_classes', type=int, default=117,
                        help="Number of verb classes")
    parser.add_argument('--pretrained', type=str, default='E:\ONGSKFYP\EoID\output\EoID_rice_cooker\checkpoint_35.pth',
    ...
    parser.add_argument('--rice_cooker_path', default='E:/ONGSKFYP/add_frames', type=str)  # 图像文件夹路径
    parser.add_argument('--coco_panoptic_path', type=str)
    parser.add_argument('--remove_difficult', action='store_true')
    parser.add_argument('--hoi_path', type=str)

    parser.add_argument('--output_dir', default='E:\ONGSKFYP\EoID\output',
...
#     global coco_instance_id_to_name  # 显式声明使用全局变量
    assert args.dataset_file in ['hico', 'vcoco', 'hoia', 'rice_cooker'], args.dataset_file
    ...
    elif args.dataset_file == 'rice_cooker':
        num_classes = 53  # 根据实际情况修改
        num_actions = 117  # 根据实际情况修改
        # rice_cooker_specific_setup()  # 如果需要针对 rice_cooker 进行特殊设置
    else:
        num_classes = 12
        num_actions = 11
    ...
    coco_instance_id_to_name = list(globals()['coco_instance_id_to_name'].values())
    ...
    for idx_box, idx_action in box_action_pairs:
        ...
        if i_cls < hoi_th or h_cls < human_th :
            continue
        pp = dict(
...
def transfer_image(img):
    ...
    # img_name = 'img_%06d.jpg' % (count())
    # i_id = hoi_interaction_names.index(hoi_list[0]['i_name'])
    img_result = viz_hoi_result(img=img, hoi_list=hoi_list)
    return {
        # 'verb_id' : i_id,
        'hoi_list' : hoi_list,
...
def main():
    ...
    if args.img_sheet is None:
        img_path_list = [
            'E:/ONGSKFYP/add_frames/test/frame_0297.png',
            'E:/ONGSKFYP/add_frames/test/frame_0404.png',
        ]
    else:
